...
def generate_cbow_data(tokenized_docs, word2idx, window=2):
    ...
    data = []
    for tokens in tqdm(tokenized_docs, desc="Extracting context-target"):
...
def train():
    ...
    # 2. Generate Data
    data = generate_cbow_data(tokenized_docs, word2idx, window=WINDOW)
    ...
    for epoch in range(EPOCHS):
        ...
        print(f"Epoch {epoch+1} Results: Train Loss: {train_loss/train_total:.4f}, Train Acc: {train_correct/train_total:.4f} | "
              f"Val Loss: {val_loss/val_total:.4f}, Val Acc: {val_correct/val_total:.4f}\n")

    # 6. Save results
    print("Saving model and embeddings...")
    torch.save(model.state_dict(), "cbow_model.pth")
    torch.save(model.embedding.weight.data.cpu(), "cbow_embeddings.pt")
    import json
