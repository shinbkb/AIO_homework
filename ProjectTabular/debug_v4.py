...
# 3. Yêu cầu mới: Bỏ luôn cột Version vì làm giảm ý nghĩa Data
...
# Bước 3: FIX DATA LEAKAGE - Tránh "Chép phao" rò rỉ target variable vào tập train/test
# 1. Cắt Train / Test NGAY TỪ ĐẦU trên Dataframe thay vì lấy log gộp
np.random.seed(42)  # cố định bộ sinh số ngẫu nhiên
...
# 2. TARGET ENCODING AN TOÀN TRÊN TẬP TRAIN (Không bị chép phao)
# Lấy Giá trung bình của Series CHỈ TRÊN df_train
series_mean_map_safe = df_train.groupby('Series')['Price'].mean()

# Áp giá trung bình này vào cả Train lẫn Test. 
# Ai bị ngoại lai (Series có ở Test nhưng ko có ở Train) thì fill bằng giá trung bình toàn chợ
df_train['Series'] = df_train['Series'].map(series_mean_map_safe)

global_mean_price = df_train['Price'].mean()
df_test['Series'] = df_test['Series'].map(series_mean_map_safe).fillna(global_mean_price)

# Xóa rác: Chắc chắn bỏ các cột string còn lại không thể đưa vào Random Forest Numpy
...
# 3. Tạo Matrix Numpy (loại bỏ cơ chế log() như XGBoost)
X_train = df_train.drop(columns=['Price']).values.astype(float)
y_train = df_train['Price'].values.astype(float)

X_test = df_test.drop(columns=['Price']).values.astype(float)
y_test = df_test['Price'].values.astype(float)

print(f"Train Dataset: X_train={X_train.shape}, y_train={y_train.shape}")
print(f"Test Dataset : X_test={X_test.shape},  y_test={y_test.shape}")
...
rf_model = RandomForestRegressor(
    ...
    max_features=1.0,     # Lấy ngẫu nhiên căn bậc 2 số features cho mỗi lần split
...
y_train_vnd = y_train
y_train_pred_vnd = y_train_pred_rf

y_test_vnd = y_test
y_test_pred_vnd = y_test_pred_rf
